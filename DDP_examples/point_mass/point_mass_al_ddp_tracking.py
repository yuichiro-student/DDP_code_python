...
epsilon = 1e-2
g = [(rad_con[0]+epsilon) ** 2 - (x[0] - center_con[0, 0]) ** 2 - (x[1] - center_con[0, 1]) ** 2]

grad_g = [[-2 * (x[0] - center_con[0, 0]), -2 * (x[1] - center_con[0, 1]), 0.0, 0.0  ]]
...
#x_ddp, u_ddp, K_ddp, u_bars, x_bars, J, norm_costgrad = ddp_ctrl_constrained(u_bar,par_ddp,par_dyn,options_ddp)
...
np.save('AL_tracking_x_even_pen.npy', x_ddp)
np.save('AL_tracking_u_even_pen.npy', u_ddp)
np.save('AL_tracking_K_even_pen.npy', K_ddp)
#plt.show()

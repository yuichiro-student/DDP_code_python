...
lims_u = 100*np.array([1, 1])
lims_l = 100*np.array([-1,-1]) #box xontrol constraints
...
K = 0.1*np.diag(np.array([100, 100, 100, 100]))
...
rad_con = np.array([0.5,0.5])
center_con = np.array([[1,0.7], [1,2]])
num_con = len(rad_con)
min_lambda_al = 1e-15
mu_max = 1e8
...
#plt.show()
cov_noise = 0.01*np.array([[1,0.01],[0.01, 1]])
x_noise, u_noise = realization_with_noise(x_ddp, u_ddp, K_ddp, par_dyn, par_ddp, options_lagr,cov_noise, num_samples = 10)
draw_graph_with_noise(x_ddp,u_ddp,x_noise,par_dyn,par_ddp,options_lagr)
plt.show()